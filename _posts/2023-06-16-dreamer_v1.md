...
### Behind the our PyTorch implementation of the State-of-the-Art RL algorithm.
...
</div><image_caption style="margin-bottom:28px; width: 100%; text-align: center; display: block;">Figure 2: How the models of the RSSM works. The recurrent model encodes the history of the episode. The representation model encodes the actual stochastic state. The transition model predicts the current stochastic state.</image_caption>
...
<img id="fig-rssm" src="{{site.baseurl}}/assets/images/dreamer_v1/rssm.png" /><image_caption style="margin-bottom:28px; width: 100%; text-align: center; display: block;">Figure 3: How the RSSM works. The three models of the RSSM iteratively compute the current and predicted latent state.</image_caption>
...
###### Actor
The actor ($a$) of Dreamer supports both continuous and discrete control. For continuous actions, it outputs the mean and standard deviation of the actions. The mean is scaled by a factor of 5, and the standard deviation is processed using a SoftPlus function to ensure non-negative values. The formulas for calculating the mean and standard deviation are not explicitly described in the paper, but can be derived from the code. The standard deviation is increased by a *raw init std* value and then transformed using [SoftPlus](https://pytorch.org/docs/stable/generated/torch.nn.Softplus.html), with a minimum amount of standard deviation added to the result. The resulting normal distribution is transformed using a hyperbolic tangent (tanh) function, and the Independent distribution is used to set the correct event shape. The mean and standard deviation for the discrete control are computed as follows:
