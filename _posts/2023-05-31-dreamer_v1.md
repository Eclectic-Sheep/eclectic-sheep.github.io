...
### Results
The results of DreamerV1 are well known and well described in its paper. Nevertheless, we want to show the results obtained with our implementation, thus demonstrating the validity of our work. Dreamer's greatest peculiarity is his ability to imagine the consequences of actions, we trained Ms Pacman for 5000000 steps with action repeat equal to 2 and max episode length equal to 1000, then we observed the behaviour of the agent and we analysed how it imagine future states. The result is shown in [Figure 7](#fig-mspac){: .fig-link} in which it can be observed that the agent can imagine the consequences of the actions it selects, in particular, it starts from the bottom-right corner of the map (yellow dot) and in the first few steps it imagines going up and "eating" the rewards.
