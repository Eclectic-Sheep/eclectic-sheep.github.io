                ...
                with torch.no_grad():
                    ...
                    if is_continuous:
                        ...
                        real_actions = np.concatenate(
                            [act.argmax(dim=-1).cpu().numpy() for act in actions], 
                            axis=-1
                        )
...
The second thing is the **agent.py** file, where we will define the **build_agent** function, which will be used to create the agent model:
...
# The following line is the one to be added
